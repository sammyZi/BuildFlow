    ...
    try {
      if (isSignUp) {
        ...
        if (data.user && !data.session) {
          ...
          return; // Don't redirect, let them see the message
        }
      } else {
        ...
        {successMessage ? (
          <div className="bg-emerald-50 border border-success/30 rounded-2xl p-6 text-center animate-fade-in-up">
            ...
            <button
              ...
              className="w-full py-3 px-4 rounded-xl bg-surface border border-border text-text-primary font-bold hover:bg-surface-alt transition-colors"
            >
              Back to Sign In
            </button>
          </div>
