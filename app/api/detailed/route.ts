...
[{"id":"tech1","question":"Short tech question?","options":["Short Opt 1","Short Opt 2","Short Opt 3","Short Opt 4"]}]`,
    ...
    if (action === 'generate_design') {
      const { text } = await generateText({
        model,
        system: 'You are an expert Software Architect. Generate a technical system design document based on the given app idea, requirements, and tech stack choices. Include System Architecture (with a Mermaid diagram), Tech Stack (Frontend, Backend, Database), Data Models, and API Endpoints. Use markdown formatting. IMPORTANT FOR MERMAID DIAGRAMS: 1. Wrap diagrams in code blocks with the \'mermaid\' language identifier. 2. For ALL nodes, if the node label contains spaces, slashes `/`, parentheses `()`, brackets `[]`, braces `{}`, quotes, or other special characters, you MUST wrap the ENTIRE label text in double quotes to prevent syntax errors. Example: `NodeId["My Node (Details)"]` or `A["Buyer/Seller Web Browser/PWA"]`. Do NOT mix quotes inside brackets without wrapping the entire label, such as `A[Buyer/Seller "Web Browser/PWA"]`. 3. For ALL edge labels containing spaces, parentheses, slashes, or special characters, you MUST wrap the edge label in double quotes using the `-->|"Edge Label"|` syntax. 4. NEVER use `--(Label)-->` or `-- Label -->` for edge labels; use standard `-->|Label|` or `-->|"Label"|` syntax only. 5. For subgraphs, the ID must be a single alphanumeric word without special characters or spaces, and the visual title must be in brackets, e.g., `subgraph InfrastructureServices ["Infrastructure & Services"]`. Ensure all Data Models are presented in standard Markdown tables with proper line breaks for each row. Do not output tables on a single line.',
